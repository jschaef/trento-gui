...
if st.session_state.get("logged_in", None):
    ...
    if projects:
        action = col1.radio("Select the action you want to perform", ["Run checks", "Manage old projects"],
            index=0, key="action", horizontal=True)
        if action == "Run checks":
            project = col2.selectbox("select the project you want to execute the tests on",
                projects, key="project",help="Select project")    
            ...
            if len(check_results) > 1:
                topic = col3.selectbox("select the topic you want to filter", 
                    ["all", "Corosync", "SBD", "Pacemaker", "OS and package versions",
                     "saptune", "Azure Fence Agent", "Filesystems", "Sapservices",
                     "SAP HANA System Replication Resource Agent ocf:suse:SAPHana"], key="topic")
                ...
                if col1.button(f"Show check results for {project}", key="run_historical_checks"):
                    vf.make_vspace(3, col1b)
                    format_output = fwo.format_output(check_results, 
                    result_container, topic, message_type)
                checks_run = 1
                ...
                if col1.button(f"Run initial checks for {project}", key="run_checks"):
                    col1b.info(f"""project {project} seems to be running the 
                        ...
                        it may take a while""")
                    ret = tcsc_checks.run_checks(workspace, username, project, result_container)
                    checks_run = 1
            if checks_run:
                vf.make_vspace(3, col1)
            ...
            if projects:
                project = col2.selectbox("select the project you want to delete", projects, key="project")
                ...
                col1.page_link("pages/6_🔨trento_container_initialization.py", label="Go to the Trento Initialization page", icon="🔨")
    else:
        col1.write("No projects found, please create a project first")
        col1.page_link(
